...
use Hexactyl\\Exceptions\Service\Helper\CdnVersionFetchingException;

class SoftwareVersionService
{
    public const VERSION_CACHE_KEY = 'Hexactyl:versioning_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://Hexactyl.io/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('Hexactyl.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('Hexactyl.cdn.url'));
