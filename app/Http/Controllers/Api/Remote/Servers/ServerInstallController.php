...
use Hexactyl\\Events\Server\Installed as ServerInstalled;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use Hexactyl\\Http\Requests\Api\Remote\InstallationDataRequest;
...
class ServerInstallController extends Controller
{
    ...
    public function store(InstallationDataRequest $request, string $uuid): JsonResponse
    {
        ...
        if ($isInitialInstall && config()->get('Hexactyl.email.send_install_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        } elseif (!$isInitialInstall && config()->get('Hexactyl.email.send_reinstall_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        }
